-- ============================================================
-- 04 — Relatório Consolidado (View do Dashboard)
-- Projeto: curso-eba-493312 | Dataset: AulasSQL
-- ============================================================
-- CONTEXTO:
--   Query que consolida as principais métricas do negócio
--   em uma única view. Usada como fonte de dados para os
...
-- 2. Receita e volume por mês (evolução 2024)
SELECT
...
-- 3. Distribuição de produtos vendidos
SELECT
    product_type,
    COUNT(*)                                                    AS qtd_vendas,
...
-- 4. View consolidada de atendimento (tabela relatorio_final)
--    Usada diretamente no dashboard como fonte de dados
SELECT
    atendente,
...
--    (base do gráfico de linhas por categoria)
SELECT
    FORMAT_DATE('%Y-%m', data_compra)  AS mes,
    product_type,
    ROUND(SUM(valor), 2)              AS receita
FROM `curso-eba-493312.AulasSQL.compras`
GROUP BY mes, product_type
ORDER BY mes, product_type;
